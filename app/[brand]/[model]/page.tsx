...
// Typical India on-road-price components as a % of ex-showroom — shown as
// clearly-labelled ESTIMATES the customer can see and adjust, never as an
// official/exact figure. Actual RTO/insurance vary by state and customer.
const DEFAULT_RTO_PCT = 9;
const DEFAULT_INSURANCE_PCT = 4;
const DEFAULT_OTHER_CHARGES = 12000;

function computeOnRoadEstimate(exShowroom: number, rtoPct: number, insurancePct: number, otherCharges: number) {
  const rto = Math.round((exShowroom * rtoPct) / 100);
  const insurance = Math.round((exShowroom * insurancePct) / 100);
  const onRoad = exShowroom + rto + insurance + otherCharges;
  return { rto, insurance, onRoad };
}
...
export default function ModelDetailPage() {
  ...
  const [rtoPct, setRtoPct] = useState(DEFAULT_RTO_PCT);
  const [insurancePct, setInsurancePct] = useState(DEFAULT_INSURANCE_PCT);
  const [otherCharges, setOtherCharges] = useState(DEFAULT_OTHER_CHARGES);
  const [downPaymentPct, setDownPaymentPct] = useState(20);
  const [tenureMonths, setTenureMonths] = useState(60);
  const [emiRoi, setEmiRoi] = useState(9.5);
      ...
      {!loading && !notFound && data && variant && (
        <main>
          ...
          <section className="section" id="price">
            <div className="container">
              <h2 className="section-title">{data.brand.name} {data.model.name} Price & EMI Calculator</h2>
              <p className="section-sub">Adjust the numbers below — these are editable estimates, not the final figure. Contact us for an exact on-road price and loan offer.</p>
              {(() => {
                const { rto, insurance, onRoad } = computeOnRoadEstimate(variant.exShowroomPrice || 0, rtoPct, insurancePct, otherCharges);
                const downPaymentAmt = Math.round((onRoad * downPaymentPct) / 100);
                const loanAmt = onRoad - downPaymentAmt;
                const emi = computeEmi(loanAmt, emiRoi, tenureMonths);
                return (
                  <div className="price-layout">
                    <div className="card side-card">
                      <h3>On-Road Price Estimate — {variant.name}</h3>
                      <div className="emi-row"><span>Ex-showroom Price</span><strong>{formatPrice(variant.exShowroomPrice)}</strong></div>
                      <div style={{ margin: '10px 0' }}>
                        <label style={{ fontSize: 11.5, color: '#8fa3ad', display: 'block', marginBottom: 4 }}>RTO — {rtoPct}%</label>
                        <input type="range" min={5} max={20} step={0.5} value={rtoPct} onChange={(e) => setRtoPct(Number(e.target.value))} style={{ width: '100%', accentColor: '#159ac4' }} />
                      </div>
                      <div style={{ margin: '10px 0' }}>
                        <label style={{ fontSize: 11.5, color: '#8fa3ad', display: 'block', marginBottom: 4 }}>Insurance — {insurancePct}%</label>
                        <input type="range" min={2} max={8} step={0.5} value={insurancePct} onChange={(e) => setInsurancePct(Number(e.target.value))} style={{ width: '100%', accentColor: '#159ac4' }} />
                      </div>
                      <div className="emi-row"><span>RTO (estimate)</span><strong>₹{rto.toLocaleString('en-IN')}</strong></div>
                      <div className="emi-row"><span>Insurance (estimate)</span><strong>₹{insurance.toLocaleString('en-IN')}</strong></div>
                      <div className="emi-row"><span>Other charges (est.)</span><strong>₹{otherCharges.toLocaleString('en-IN')}</strong></div>
                      <div className="emi-row" style={{ fontSize: 14 }}><span>On-Road Price (estimate)</span><strong>₹{onRoad.toLocaleString('en-IN')}</strong></div>
                      <p style={{ marginTop: 10, fontSize: 11.5, color: '#8fa3ad' }}>Actual RTO/insurance vary by state and customer profile — this is only an estimate to help you plan.</p>
                    </div>
                    <div className="card side-card">
                      <h3>EMI Calculator</h3>
                      <div style={{ margin: '10px 0' }}>
                        <label style={{ fontSize: 11.5, color: '#8fa3ad', display: 'block', marginBottom: 4 }}>Down Payment — {downPaymentPct}% (₹{downPaymentAmt.toLocaleString('en-IN')})</label>
                        <input type="range" min={0} max={90} step={5} value={downPaymentPct} onChange={(e) => setDownPaymentPct(Number(e.target.value))} style={{ width: '100%', accentColor: '#159ac4' }} />
                      </div>
                      <div style={{ margin: '10px 0' }}>
                        <label style={{ fontSize: 11.5, color: '#8fa3ad', display: 'block', marginBottom: 4 }}>Tenure — {tenureMonths} months</label>
                        <input type="range" min={12} max={84} step={6} value={tenureMonths} onChange={(e) => setTenureMonths(Number(e.target.value))} style={{ width: '100%', accentColor: '#159ac4' }} />
                      </div>
                      <div style={{ margin: '10px 0' }}>
                        <label style={{ fontSize: 11.5, color: '#8fa3ad', display: 'block', marginBottom: 4 }}>Interest Rate — {emiRoi}% p.a.</label>
                        <input type="range" min={7} max={16} step={0.25} value={emiRoi} onChange={(e) => setEmiRoi(Number(e.target.value))} style={{ width: '100%', accentColor: '#159ac4' }} />
                      </div>
                      <div className="emi-row"><span>Loan Amount</span><strong>₹{loanAmt.toLocaleString('en-IN')}</strong></div>
                      <div className="emi-row" style={{ fontSize: 16 }}><span>Estimated EMI</span><strong style={{ color: '#159ac4' }}>₹{emi.toLocaleString('en-IN')}/mo</strong></div>
                      <button className="btn full" onClick={() => setModalOpen(true)}>Apply for Finance</button>
                    </div>
                  </div>
                );
              })()}
